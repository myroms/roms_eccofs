...
do
  case "$1" in
    ...
    * )
      ...
      echo "                  For example:  cbuild_split.sh -p FFLAGS"
      echo ""
...
if [ "${USE_DEBUG:-x}" = "on" ]; then
  export        BUILD_DIR=${MY_PROJECT_DIR}/CBuild_splitG
else
  if [ "${USE_MPI:-x}" = "on" ]; then
    export      BUILD_DIR=${MY_PROJECT_DIR}/CBuild_splitM
  else
    export      BUILD_DIR=${MY_PROJECT_DIR}/CBuild_split
  fi
fi
